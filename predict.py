...
from utils import metrics, pallete
...
def sliding_predict(model, image, num_classes, flip=True):
    ...
    tile_size = (image_size[2]//2.5, image_size[3]//2.5)
    overlap = 1/3
...
def multi_scale_predict(model, image, scales, num_classes, device, flip=True):
...
def save_images(image, mask, output_path, image_file, pallete):
	# Saves the image, the model output and the results after the post processing
    w, h = image.size
    colorized_mask = colorize_mask(mask, pallete)
    output_im = Image.new('RGB', (w*2, h))
    output_im.paste(image, (0,0))
    output_im.paste(colorized_mask, (w,0))
    output_im.save(os.path.join(output_path, image_file))
    ...
    with torch.no_grad():
        ...
        for img_file in tbar:
            ...
            if args.whole:
                ...
                prediction = sliding_predict(model, image, num_classes)
            prediction = F.softmax(torch.from_numpy(prediction), dim=0).cpu().numpy()

            if args.save_images:
                save_images(image, prediction.argmax(0), args.output, img_file, pallete)

def parse_arguments():
    parser = argparse.ArgumentParser(description='Inference')
    parser.add_argument('-c', '--config', default='VOC',type=str,
                        help='The config used to train the model')
    parser.add_argument('-w', '--whole', default=False, type=bool,
                        help='Use multipple scales or sliding')
